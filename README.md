...
既存の保存済みキーマップがある場合はデフォルト設定より優先されるため、Vial 側で対象キーを
`LT(4,Semicolon)` に設定するか、キーマップを初期化してください。
...
- RMK 0.8.2 の PMW3610 は通常レイヤーではカーソル入力として扱い、`scroll` レイヤー（layer 4）
  が有効な間は X/Y を縦ホイール／横パンへ変換します。Base の JIS `;` はタップで `;`、
  ホールドで `scroll` レイヤーを有効にします。
