...
pub struct KeyMap<'a, const ROW: usize, const COL: usize, const NUM_LAYER: usize, const NUM_ENCODER: usize = 0> {
    ...
    /// Current mouse button state shared with pointing-device processors.
    mouse_buttons: u8,
    ...
    KeyMap<'a, ROW, COL, NUM_LAYER, NUM_ENCODER>
{
    ...
    /// Get the current mouse button state for reports generated by pointing devices.
    pub(crate) fn mouse_buttons(&self) -> u8 {
        self.mouse_buttons
    }

    /// Update the current mouse button state shared with pointing devices.
    pub(crate) fn set_mouse_buttons(&mut self, buttons: u8) {
