    ...
    Storage<F, ROW, COL, NUM_LAYER, NUM_ENCODER>
{
    ...
    async fn check_enable(&mut self) -> bool {
        ...
        .await
        {
            // A firmware/configuration change must not reuse the old keymap,
            // behavior settings, or BLE state. BUILD_HASH is regenerated by
            // the RMK build script when keyboard.toml changes.
            if storage_config_is_current(&config) {
                return true;
            }
        }
        false
...
fn storage_config_is_current(config: &LocalStorageConfig) -> bool {
    config.enable && config.build_hash == BUILD_HASH
}
...
mod tests {
    ...
    #[test]
    fn storage_config_requires_enabled_current_build() {
        assert!(storage_config_is_current(&LocalStorageConfig {
            enable: true,
            build_hash: BUILD_HASH,
        }));
        assert!(!storage_config_is_current(&LocalStorageConfig {
            enable: true,
            build_hash: BUILD_HASH.wrapping_add(1),
        }));
        assert!(!storage_config_is_current(&LocalStorageConfig {
            enable: false,
            build_hash: BUILD_HASH,
